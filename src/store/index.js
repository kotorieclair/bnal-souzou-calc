...
export default {
  init({ bungo, cards }) {
    if (!bungo || !cards) {
      throw new Error(
        'Store: init - static data for validation must be provided!'
      )
    }
      ...
      const actions = {
        ...
        setCardLv(cardLv) {
          if (cardLv !== '') {
            if (!state.cardId) {
              throw new Error(
                'Store.action: setCardLv - set cardId before setting cardLv!'
              )
            }
        ...
        setBaseStatus(key, val) {
          if (!statusData.base.hasOwnProperty(key)) {
            throw new Error(
              'Store.action: setBaseStatus - unknown baseStatus key!'
            )
          }
          if (val !== '' && !Number.isInteger(val)) {
            throw new Error(
              `Store.action: setBaseStatus - ${key} must be an integer!`
            )
          }
          if (val <= -1) {
            state[key] = 1
